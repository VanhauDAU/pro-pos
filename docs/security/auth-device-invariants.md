...
PBKDF2-HMAC-SHA256 dùng Web Crypto, salt riêng, pepper server-side; hash lưu algorithm,
work factor và versions. Work factor được benchmark trên deployed Worker. Workers Free chỉ được
chấp nhận nếu không có error 1102 và P99 auth CPU ≤ 8 ms. Không hạ security baseline để giữ free;
mặc định nâng Workers Paid nếu không đạt.
