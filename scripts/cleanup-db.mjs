...
const queries = [
  ...
  `DELETE FROM service_requests WHERE (status IN ('COMPLETED', 'CANCELLED') AND created_at < ${cutoffMs}) OR order_id IN (SELECT id FROM orders WHERE status IN ('PAID', 'CANCELLED'));`,
  `DELETE FROM accept_guest_order_request_commands WHERE guest_request_id IN (SELECT id FROM guest_order_requests WHERE (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND created_at < ${cutoffMs}) OR (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND order_id IN (SELECT id FROM orders WHERE status IN ('PAID', 'CANCELLED'))));`,
  `DELETE FROM reject_guest_order_request_commands WHERE guest_request_id IN (SELECT id FROM guest_order_requests WHERE (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND created_at < ${cutoffMs}) OR (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND order_id IN (SELECT id FROM orders WHERE status IN ('PAID', 'CANCELLED'))));`,
  `DELETE FROM guest_order_request_items WHERE request_id IN (SELECT id FROM guest_order_requests WHERE (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND created_at < ${cutoffMs}) OR (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND order_id IN (SELECT id FROM orders WHERE status IN ('PAID', 'CANCELLED'))));`,
  `DELETE FROM guest_order_requests WHERE (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND created_at < ${cutoffMs}) OR (status IN ('ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED') AND order_id IN (SELECT id FROM orders WHERE status IN ('PAID', 'CANCELLED')));`,
